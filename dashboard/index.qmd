...
::: {.valuebox icon="droplet-fill" color="#e3f2fd" title="Cobertura Nacional (SIODS)"}
64%
:::

### Column {width=15%}

::: {.valuebox icon="houses-fill" color="#e74c3c" title="Municipios Rurales"}
55%
:::

### Column {width=15%}

::: {.valuebox icon="exclamation-triangle-fill" color="#2c3e50" title="Desigualdad Hídrica Rural"}
4.6x
:::
...
```{python}
#| title: "Carencia de Agua Entubada en los 2,469 Municipios de México - 2020"
import numpy as np
import base64
...
# Bins que superan el promedio nacional oficial
n_bins = len(bin_centers)
target_bins = [i for i, xc in enumerate(bin_centers) if xc >= carencia_oficial]
...
for bi in target_bins:
    ...
    for clasif in ['Rural', 'Urbano']:
        # Ocultar solo el icono Rural en el primer bin para evitar que se encime con la línea punteada
        if bi == target_bins[0] and clasif == 'Rural':
            continue
...
```{python}
#| title: "Distribución de Municipios en México - 2020"
...
::: {.card title="Déficit de Agua vs. Pobreza Extrema en México (2,469 Municipios) - 2020"}
...
```{python}
#| title: "Cobertura de Agua(%) en México por Estado - 2020"
#| warning: false
#| message: false
import json
...
```{python}
#| title: "Municipios con Mayor Rezago de Infraestructura Hidríca en México - 2020"
...
[Conoce las historias y las acciones ➔](#accion-positiva){.btn-cta}
:::


# LA VOZ Y LA ACCIÓN {#accion-positiva .seccion-humana .scrolling}
...
::: {.card-tiempo}
::: {.card-tiempo-main}
[El costo de la búsqueda del agua]{.card-tiempo-heading}
...
```{python}
#| title: "ONGs con impacto en infraestructura hídrica en México - 2020"
...
df_mapa['Impactos'] = df_mapa['Impactos'].fillna('Otras intervenciones')
...
def clasificar_intensidad(ong_count):
    if ong_count == 0:
        ...
        return 'Presencia 1 ONG'
    elif ong_count == 2:
        return 'Presencia 2 ONGS'
    else:
        return 'Alto Impacto 3+ ONGS'
...
color_map_ongs = {
    'Sin dato 0': '#d9ebf7',            
    'Presencia 1 ONG': '#aed6f1',               # Azul claro suave
    'Presencia 2 ONGS': '#3498db',        # Azul medio vibrante
    'Alto Impacto 3+ ONGS': '#1a5276'            # Azul profundo institucional
}
...
fig_ongs = px.choropleth(
    ...
    featureidkey='properties.clvedo', 
    color='Nivel_Impacto',
    category_orders={'Nivel_Impacto': ['Sin dato 0', 'Presencia 1 ONG', 'Presencia 2 ONGS', 'Alto Impacto 3+ ONGS']},
...
hover_colors = {
    'Sin dato 0': '#95a5a6',
    'Presencia 1 ONG': '#5dade2',
    'Presencia 2 ONGS': '#2471a3',
    'Alto Impacto 3+ ONGS': '#1a5276'
}
...
[**Cántaro Azul** — Ecotecnias de purificación.]{.aliado-texto}
:::
...
::: {.citas-lista}
- **Testimonio 1** — (Video de YouTube – Azteca Noticas) YouTube. (14 mar 2023). [Video]. YouTube. https://youtu.be/mrqfSbxZgnU»*. Fotografía: José Jiménez y Luis Gress / Azteca Noticias, 2024. [Fuente](https://www.youtube.com/watch?v=mrqfSbxZgnU){target="_blank"}
- **Testimonio 2** — (Artículo periodístico) Hernández, C. E. (2025, marzo 24). ¿Cómo es vivir sin agua? La realidad de los pueblos del norte de Sinaloa. Revista Espejo. https://revistaespejo.com/2025/03/24/como-es-vivir-sin-agua-la-realidad-de-los-pueblos-del-norte-de-sinaloa/»*. Espejo: Las cosas como son. Fotografía: Isla Urbana. [Fuente](https://revistaespejo.com/2025/03/24/como-es-vivir-sin-agua-la-realidad-de-los-pueblos-del-norte-de-sinaloa/){target="_blank"}
- **Testimonio 3** — (Página web / organización) Isla Urbana. (s.f.). Historias de agua. https://islaurbana.org/historias-de-agua/»*. Isla Urbana, Historias de agua. Fotografía: Espejo: Las cosas como son. [Fuente](https://islaurbana.org/historias-de-agua/){target="_blank"}
- **Testimonio 4** — (Organismo internacional – nota web) Programa de las Naciones Unidas para el Desarrollo (PNUD). (5 ene 2023). Más de 6 mil familias de 44 localidades rurales e indígenas en el sureste de México mejoran su acceso a agua y saneamiento. https://www.undp.org/es/mexico/noticias/mas-de-6-mil-familias-de-44-localidades-rurales-e-indigenas-en-el-sureste-de-mexico-mejoran-su-acceso-agua-y-saneamiento»*. PNUD México, enero 2023. Fotografía: PNUD México. [Fuente](https://www.undp.org/es/mexico/noticias/mas-de-6-mil-familias-de-44-localidades-rurales-e-indigenas-en-el-sureste-de-mexico-mejoran-su-acceso-agua-y-saneamiento){target="_blank"}
:::
:::
...
[Explora los Datos ➔](#auditoria-de-datos){.btn-cta}
:::

:::


# AUDITORÍA DE DATOS {#auditoria-de-datos .scrolling}
